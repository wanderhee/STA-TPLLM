...
# PEMS04.npz ['data'].shape: (16992,307,3)  59天，307个传感器，（交通流量，平均速度，平均占有率）
...
def search_data(sequence_length, num_of_depend, label_start_idx,
    ...
    # 这里把区间限制2重新写了一遍，其实可以不用写
    if label_start_idx + num_for_predict > sequence_length:
        return None

    x_idx = []

    # num_of_depend即num_of_hours/days/weeks
    for i in range(1, num_of_depend + 1):
        # 开始位置：从points_per_hour * units * i开始
        # units：每个num_of_depend的小时数：week: 7 * 24, day: 24, recent(hour): 1
...
def get_sample_indices(data_sequence, num_of_weeks, num_of_days, num_of_hours,
    ...
    # label_start_idx：找一个label，label前12（或其他）个数据用来预测，后12个进行预测
    # 因此有两个限制：label_start_idx-12>=0；label+12<=数据长度
...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    # num_of_weeks, num_of_days, num_of_hours: 值为int,在conf中修改: 用几周/几天/几小时来预测
    # 例如num_of_hours=1：用12个step来预测，即时间窗口大小为12
    # num_for_predict：预测接下来多少个step的数据，如12即预测未来1小时的数据

    # 数据处理：将x和y处理为希望预测的格式，即时间窗口类型
    '''
    Parameters
    ----------
    ...
    '''
    data_seq = np.load(graph_signal_matrix_filename)['data']  # (sequence_length, num_of_vertices, num_of_features) 原始数据序列
    ...
    for idx in range(data_seq.shape[0]):
        ...
        if num_of_hours > 0:
            # 维度转换为(1,307,3,12)
            hour_sample = np.expand_dims(hour_sample, axis=0).transpose((0, 2, 3, 1))  # (1,N（节点）,F（特征）,T（step）)
        ...
        # 所有训练样本 all_samples长度为16969，比原始长度少了23（上下界）
        all_samples.append(
